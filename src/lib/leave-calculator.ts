...
const ADVANCE_LEAVE_MULTIPLIER = 4;
...
export function calculateLeave(
  ...
  return {
    ...
    breakdown: {
      totalDays:
        breakdown.weekdaysCount +
        breakdown.weekendsSkipped +
        breakdown.holidaysSkipped,
...
export function getSubmissionStatus(lastSubmitDate: Date | null): {
  ...
  if (isSubmissionLate(lastSubmitDate)) {
    return {
      status: "late",
      message:
        "⚠️ Too late to request this leave — the submission deadline has passed.",
    };
  }
